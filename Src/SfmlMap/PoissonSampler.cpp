...
bool PoissonSampler::isSampleValid(DbPoint& p ,std::vector<DbPoint>& points)
{
    ...
    int cellRow = backingGrid.GetCellRow(p.x, p.y);
    int cellCol = backingGrid.GetCellColumn(p.x, p.y);
...
{
    double px = randomDouble(0, backingGrid.height);
    double py = randomDouble(0, backingGrid.width);
